...
static Vector4f GetNearestPixelColor(
    const          Vector4f*                 dynarg_texture_buffer,
...
SHARED_INLINE float4 GetNearestPixelColor(
    const __global float4* restrict          dynarg_texture_buffer,
...
#endif
{
    ...
    return INDEX_SAFE(dynarg_texture_buffer, textureOffset);
}
...
#endif
{
    ...
    result = (INDEX_SAFE(dynarg_texture_buffer, X0Y0) * u_opposite + INDEX_SAFE(dynarg_texture_buffer, X1Y0) * u_ratio) * v_opposite +
        (INDEX_SAFE(dynarg_texture_buffer, X0Y1) * u_opposite + INDEX_SAFE(dynarg_texture_buffer, X1Y1) * u_ratio) * v_ratio;
    return result;
}
...
)
{
    ...
    if (useNearest)
    {
        return GetNearestPixelColor(dynarg_texture_buffer, saneUVs, matProperty, gBufferFiltering KERNEL_VALIDATOR_BUFFERS);
    }
    else
    {
        return GetBilinearFilteredPixelColor(dynarg_texture_buffer, saneUVs, matProperty, gBufferFiltering KERNEL_VALIDATOR_BUFFERS);
    }
}

static float4 FetchAlbedoTextureFromMaterialAndUVs(
    const __global uchar4* restrict             albedoTextures_buffer,
    const          float2                       textureUVs,
    const          MaterialTextureProperties    matProperty,
    const          bool                         gBufferFiltering
