...
)
{
    ...
    const float maxt = (light.lightType == 1) ? 1000000.0f : length(light.pos.xyz - pushOffPos);
    ...
    if (light.lightType != 1)
    {
        ...
        const float pushOffDist = length(pushOffPos - surfacePosition);
        const float surfToLightDist = length(light.pos.xyz - surfacePosition);
        if (pushOffDist > surfToLightDist)
        {
            Ray_SetInactive(dstRay);
            return;
        }
