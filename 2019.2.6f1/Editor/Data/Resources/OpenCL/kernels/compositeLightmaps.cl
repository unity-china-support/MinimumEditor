...
__kernel void compositingMarkupInvalidTexels(
    ...
    __global int* const         indirectSampleCountBuffer,
...
__kernel void compositingDirect(
    __write_only image2d_t  dynarg_dstTile,
    INPUT_BUF(1, float4,    outputDirectLightingBuffer),
    INPUT_BUF(2, int,       directSampleCountBuffer   ),
    INPUT_EL( 3, int2  ,    tileCoordinates           ),
    INPUT_EL( 4, int   ,    lightmapSize              )
    KERNEL_VALIDATOR_BUFFERS_DEF)
{
    // Coordinates in tile space
    int2 tileThreadId = (int2)(get_global_id(0), get_global_id(1));
...
__kernel void compositingShadowMask(
    __write_only image2d_t      dynarg_dstTile,
    __global const int* restrict directSampleCountBuffer,
...
__kernel void compositingIndirect(
    __write_only image2d_t  dynarg_dstTile,
    INPUT_BUF(1, float4 ,   outputIndirectLightingBuffer      ),
    INPUT_BUF(2, int,       indirectSampleCountBuffer         ),
    INPUT_BUF(3, float4 ,   outputEnvironmentLightingBuffer   ),
    INPUT_BUF(4, int    ,   environmentSampleCountBuffer      ),
    INPUT_EL( 5, float  ,   indirectIntensity                 ),
    INPUT_EL( 6, int2   ,   tileCoordinates                   ),
    INPUT_EL( 7, int    ,   lightmapSize                      )
    KERNEL_VALIDATOR_BUFFERS_DEF)
{
    // Coordinates in tile space
    int2 tileThreadId = (int2)(get_global_id(0), get_global_id(1));
